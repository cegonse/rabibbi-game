...
void gui_init(gui_state_t *state, u8 chrPalIndex)
{
	...
	state->player_two_bar[0] = SPR_addSprite(&gui_hud_lifebar_begin_filled_def, 
		GUI_HP_PLAY_TWO_X + GUI_HP_BAR_SPACING, GUI_HP_PLAY_TWO_Y, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
	...
	for (u8 i = 1; i < GUI_HP_BAR_SEGMENTS - 1; ++i)
	{
		state->player_one_bar[i] = SPR_addSprite(&gui_hud_lifebar_segment_filled_def, 
			GUI_HP_PLAY_ONE_X + GUI_HP_BAR_SPACING + i * GUI_HP_BAR_SEGMENT_SIZE, GUI_HP_PLAY_ONE_Y, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
		...
		state->player_two_bar[i] = SPR_addSprite(&gui_hud_lifebar_segment_filled_def, 
			GUI_HP_PLAY_TWO_X + GUI_HP_BAR_SPACING + i * GUI_HP_BAR_SEGMENT_SIZE, GUI_HP_PLAY_TWO_Y, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
	...
	state->player_one_bar[GUI_HP_BAR_SEGMENTS - 1] = SPR_addSprite(&gui_hud_lifebar_end_filled_def, 
		...
		TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
	...
	state->player_one_rabbit = SPR_addSprite(&gui_hud_rabbit_def, GUI_HP_PLAY_ONE_X, GUI_HP_PLAY_ONE_Y, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
	state->player_two_rabbit = SPR_addSprite(&gui_hud_rabbit_def, GUI_HP_PLAY_TWO_X, GUI_HP_PLAY_TWO_Y, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));

	state->carrot = SPR_addSprite(&gui_hud_carrot_def, GUI_CARROT_X, GUI_CARROT_Y, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
	state->carrot_left_number = SPR_addSprite(&gui_hud_number_0_def, GUI_CARROT_X + GUI_CARROT_LEFT_NUMBER_X_OFFSET, 
		GUI_CARROT_Y + GUI_CARROT_LEFT_NUMBER_Y_OFFSET, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
	state->carrot_right_number = SPR_addSprite(&gui_hud_number_0_def, GUI_CARROT_X + GUI_CARROT_RIGHT_NUMBER_X_OFFSET, 
		GUI_CARROT_Y + GUI_CARROT_RIGHT_NUMBER_Y_OFFSET, TILE_ATTR(PAL0 + chrPalIndex, FALSE, FALSE, FALSE));
